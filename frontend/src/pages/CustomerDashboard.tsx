...
import { FaCar, FaMotorcycle, FaBox, FaTruck, FaTruckPickup, FaWheelchair } from "react-icons/fa";
...
type RideStatus = "pending" | "accepted" | "cancelled" | "done" | "arrived" | "in_progress" | "scheduled" | "no_show" | null;
...
export default function CustomerDashboard() {
  ...
  const [vehicleType, setVehicleType] = useState<keyof typeof VEHICLE_TYPE_LABELS | "">("");
  ...
  useEffect(() => {
    ...
    if (!rideId || !(chatOpen && (activeRide.status === "accepted" || activeRide.status === "in_progress"))) {
      setChatMessages([]);
      return;
    }
  ...
  async function handleRequestRide() {
    if (!pickupLocation || !destination || !vehicleType || !destinationName) {
      setErrorMsg("Pickup, destination, destination name, and vehicle type required.");
      setWaiting(false);
      return;
    }
    const token = localStorage.getItem("token");
        ...
        {(activeRide.status === "accepted" || activeRide.status === "in_progress") && (
          <>
            ...
            <div style={{ margin: 10 }}>
              ...
              <b>Vehicle:</b> {activeRide.driver?.vehicleType || VEHICLE_TYPE_LABELS[activeRide.vehicleType]?.label}
            </div>
            <button
              style={{
                background: "#f44336",
  ...
  // --- UI: Ride done/cancelled, show rating and allow new ride ---
        ...
        <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: 16, marginTop: 10 }}>
          {Object.entries(VEHICLE_TYPE_LABELS).map(([type, info]) => (
            <button
              ...
              type="button"
              style={{
                border: vehicleType === type ? "2px solid #1976D2" : "2px solid #ccc",
                ...
                cursor: "pointer",
                outline: "none",
                boxShadow: vehicleType === type ? "0 0 8px #1976D2" : "0 1px 3px #eee",
