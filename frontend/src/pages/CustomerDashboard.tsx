...
// VEHICLE OPTIONS (case-insensitive)
...
export default function CustomerDashboard() {
  ...
  const [editScheduledMode, setEditScheduledMode] = useState(false);
          ...
          if (
            ...
            if (data.rideStatus === "scheduled") {
              ...
              if (data.scheduledAt && data.timeZone) {
                const localDT = DateTime.fromISO(data.scheduledAt, { zone: "utc" }).setZone(data.timeZone);
                setSchedDatetime(localDT.toFormat("yyyy-MM-dd'T'HH:mm"));
              }
            }
          }
        }
        ...
        for (const el of data.elements) {
          ...
          if (type && icon) {
            locations.push({
              type,
              name:
                el.tags.name || type.charAt(0).toUpperCase() + type.slice(1),
              lat: el.lat,
              lng: el.lon,
              phone: el.tags.phone,
              icon,
  ...
  function openEditScheduledModal() {
    setEditScheduledMode(true);
    setEditingRideId(rideId);
    setSchedVehicleType(schedVehicleType || vehicleType || "");
    setSchedDestinationName(schedDestinationName || "");
    setSchedNote(schedNote || "");
    setSchedDatetime(schedDatetime || "");
    setSchedError(null);
    setScheduledModalOpen(true);
  }
  ...
  async function handleConfirmScheduledRide() {
    const isEditing = editScheduledMode && editingRideId;
    if (
      !userLocation ||
      !schedDatetime ||
    ...
    try {
      ...
      if (isEditing) {
        res = await fetch(
          `${API_URL}/api/rides/schedule/${editingRideId}`,
          {
            method: "PUT",
            headers: {
  ...
  // --- CHAT LOGIC ---
  useEffect(() => {
    if (!rideId || !(rideStatus === "accepted" || rideStatus === "in_progress"))
      return;
    let polling = true;
  ...
  // --- CANCEL/DONE/REQUEST/RESET LOGIC ---
  ...
  async function handleRequestRide() {
    if (
      rideId &&
      ["pending", "accepted", "in_progress", "scheduled"].includes(rideStatus || "")
    ) {
      setError(
  ...
  // 1. Ride In Progress/Scheduled
  if (
    (rideStatus === "pending" && pickupSet && rideId) ||
    ["accepted", "in_progress", "scheduled"].includes(rideStatus || "")
  ) {
    return (
      <div style={{ padding: 24, textAlign: "center" }}>
        ...
        {driverInfo && ["accepted", "in_progress"].includes(rideStatus || "") && (
        ...
        {["pending", "accepted", "in_progress"].includes(rideStatus || "") && (
          <div style={{ marginTop: 18 }}>
            <button
              disabled={waiting}
              style={{
        ...
        {["accepted", "in_progress"].includes(rideStatus || "") && rideId && (
          <div
            style={{
              margin: "32px auto",
              display: "flex",
          ...
          >
            ...
            >
              <h3 style={{ marginBottom: 16 }}>
                {editScheduledMode ? "Edit Scheduled Ride" : "Schedule a Ride"}
              </h3>
  ...
  if (showRating && showDoneActions && rideId) {
    return (
      <div style={{ padding: 24, textAlign: "center" }}>
        ...
        >
          <RateDriver rideId={rideId} onRated={handleReset} />
        </div>
      </div>
    );
  }
  ...
  // 4. Main UI: Map, Emergency, Vehicle, Request, Schedule
  return (
